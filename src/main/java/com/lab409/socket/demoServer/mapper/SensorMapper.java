...
public interface SensorMapper {
    ...
    public List<Sensor> getManyByGroupIdAndType(@Param("id") Long id, @Param("type") SensorType type);


    @Insert("insert into sensor(group_id, type, descr ,host, port, msg, state, time, `interval`) " +
            "VALUES(#{sensorGroup.id}, #{type}, #{descr}, #{host}, #{port}, #{latestMsg}, #{state}, #{changedTime}, #{interval})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    void insert(Sensor sensor);

    @Insert("insert into sensor(group_id, type) values(#{sensorGroup.id}, #{type})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
